...
const MainNavbar = () => {
  ...
  return (
    <nav className="w-full flex flex-row items-center justify-between bg-primary-800 px-4 md:px-8 h-16 shadow-soft sticky top-0 z-50">
      <a href="/" className="font-bold text-xl md:text-2xl text-white tracking-tight">
        InternConnect
      </a>
      <div className="flex flex-row gap-6 items-center">
        ...
        <div className="hidden md:flex flex-row items-center gap-6">
          {user && user.role === 'admin' && (
            <a href="/admin" className={navLinkClass}>Admin</a>
          )}
          {(user && user.role === 'admin') || (org && org.role === 'org') ? (
            <a href="/org-admin" className={navLinkClass}>Org Admin</a>
          ) : null}
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-40 md:hidden" onClick={closeMenu}>
          <div className="fixed top-0 right-0 w-72 max-w-[85vw] h-full bg-primary-800 shadow-2xl p-6 flex flex-col gap-6 pt-16" onClick={(e) => e.stopPropagation()}>
            ...
            {user && user.role === 'admin' && (
              <a href="/admin" className={navLinkClass} onClick={closeMenu}>Admin</a>
            )}
            {(user && user.role === 'admin') || (org && org.role === 'org') ? (
              <a href="/org-admin" className={navLinkClass} onClick={closeMenu}>Org Admin</a>
            ) : null}
